...
def calculate_features(df):
    ...
    #mean_intra_word_iki = df_filtered.groupby(['id', 'word_count'])
    #['down_time_diff'].mean().reset_index()

    # Aggregate this feature at the 'id' level to match the granularity of your features DataFrame
    #mean_intra_word_iki = mean_intra_word_iki.groupby('id')
    #['down_time_diff'].mean().reset_index(name='mean_intra_word_iki')

    # Calculate the standard deviation of down_time_diff within each word for each id
    #std_intra_word_iki = df_filtered.groupby(['id', 'word_count'])
    #['down_time_diff'].std().reset_index()

    # Aggregate this feature at the 'id' level to match the granularity of your features DataFrame
    #std_intra_word_iki = std_intra_word_iki.groupby('id')
    #['down_time_diff'].std().reset_index(name='std_intra_word_iki')
